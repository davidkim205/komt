...
bool check_end_of_text(std::string input) {
    size_t length = input.length();
    size_t firstPos = input.find("### Response:");
    if (firstPos != std::string::npos) {
        size_t secondPos = input.find("###", firstPos + 3);
...
int main(int argc, char ** argv) {
    ...
    while ((n_remain != 0 && !is_antiprompt) || params.interactive) {
        ...
        if (true) {
            for (auto id : embd) {
                output_string += llama_token_to_str(ctx, id);
            }
        ...
        if (!embd.empty() && check_end_of_text(output_string) && !(params.instruct || params.interactive)) {
            fprintf(stderr, " check_end_of_text [end of text]\n");
            break;
        }
